...
namespace DuckyBot.Core.Modules.Commands
{
    ...
    //SocketCommandContext allows access to the message, channel, server and user the command was invoked by, so works as the base as to how the bot knows where to reply/what user used which command, etc.
    {
        ...
        public async Task Delete(int num) // command async task that takes in a parameter (remainder represents a space between the command and the parameter)
        {
            if (((SocketGuildUser)Context.User).Roles.Any(r => r.Name == "Mod"))
            {
                await ((SocketTextChannel) Context.Message.Channel).DeleteMessagesAsync(await Context.Message.Channel.GetMessagesAsync(num + 1).FlattenAsync());

                var notify = await ReplyAsync($"Successfully deleted `{num}` messages."); // notify user of message deletion success
                await Task.Delay(3000).ConfigureAwait(false);
                await ((SocketTextChannel)Context.Message.Channel).DeleteMessageAsync(notify);
            }
            else
            {
                var notify = await ReplyAsync("You are not a TDP Moderator and therefore cannot use this command!"); // notify user of message deletion success
                await Task.Delay(3000).ConfigureAwait(false);
